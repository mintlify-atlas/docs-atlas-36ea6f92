---
title: "CAI-Compliant Tax Invoicing in Credith"
sidebarTitle: "CAI Invoicing"
description: "CAI (Código de Autorización de Impresión) is Honduras's government-issued invoice authorization system. Credith validates every sale against active CAI ranges."
---
...
| `current_number` | INTEGER | Offset from `min_range` — how many bills have already been issued. Starts at `0`. |
| `expiration_date` | DATE | Independent expiry date for this range (from the `schema.md` table definition). |
...
During bill creation (`POST /api/bills`), the system acquires a `SELECT ... FOR UPDATE` lock on the active CAI Range row and atomically increments `current_number`:
