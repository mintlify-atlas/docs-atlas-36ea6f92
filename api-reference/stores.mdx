...
sidebarTitle: "Stores"
description: "Create and manage store locations. Each store belongs to a company and can be independently activated or deactivated. Products and users are assigned per store."
---
...
Read endpoints (`GET`) are public. Write operations require a valid session with an **Admin** or **Owner** role.
...
Admin users can be assigned to a specific store so they manage that location. The `admin-stores` resource also exposes two utility endpoints restricted to the **Owner** role.
