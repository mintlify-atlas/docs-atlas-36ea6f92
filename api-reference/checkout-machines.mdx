...
<Warning>
  Each user can only be assigned to **one** active checkout machine. Attempting to associate a user who already has a `checkoutMachineId` set on their account — whether via creation or the associate-user endpoint — will return a **400 Bad Request** error. Ensure the user is unlinked from any existing machine before reassigning.
</Warning>
