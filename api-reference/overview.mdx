---
title: "Credith REST API Overview and Base Configuration"
sidebarTitle: "Overview"
description: "The Credith API is a RESTful JSON service running on Express. All endpoints are prefixed with /api and return JSON responses with standard HTTP status codes."
---
...
| `authMiddleware` | 3 user routes only | Reads `req.cookies.token`, calls `verifyToken()`, and attaches the decoded payload to `req.user`. Returns `401` if the token is missing or invalid |
