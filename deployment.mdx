---
title: "Deploy Credith in Production Using Docker Compose"
...
<Steps>
  ...
  <Step title="Migrations and Base Data Run Automatically">
    The `credith_service` container's startup command runs `pnpm migrate` and `pnpm seed:prd` automatically before the API process launches. No manual migration or seeding steps are required on a fresh deployment.
  ...
  <Step title="Access the Running Application">
    ...
    | Swagger UI | [http://your-host:3000/api-docs](http://your-host:3000/api-docs) | Available while `NODE_ENV=development`; disable by setting `NODE_ENV=production` |
